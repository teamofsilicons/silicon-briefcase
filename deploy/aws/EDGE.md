...
1. Create a CloudFormation change set for `silicon-briefcase-edge` using
...
4. Change only the `backend.briefcase.teamofsilicons.com` CNAME to
...
The common and known-bad protections stay enabled. Body size and body-XSS
matches are counted and blocked again except for POST /api/v1/uploads and
POST /api/v1/obo/files on backend.briefcase.teamofsilicons.com. All other
body-threat, query, cookie, header and rate protections remain enforced.
Upload contents are still handled by the application's access and storage
controls; the exception does not create a broad Allow rule.
