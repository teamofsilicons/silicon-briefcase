...
const origin = "https://docs.briefcase.teamofsilicons.com";
const escape = (value) =>
...
for (const file of documents) {
  ...
  renderer.link = ({ href, title, tokens }) => {
    ...
    if (!/^(?:[a-z]+:|\/|#)/i.test(href)) {
      ...
      if (documents.includes(inDocs))
        ...
        destination =
          "https://github.com/teamofsilicons/silicon-briefcase/blob/main/" +
  ...
  const nav = navigation
    ...
    .join("");
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)} · Briefcase Docs</title><meta name="description" content="Silicon Briefcase 1.0 documentation: ${escape(title)}"><link rel="canonical" href="${url}"><meta property="og:title" content="${escape(title)} · Briefcase Docs"><meta property="og:url" content="${url}"><link rel="icon" href="/favicon.svg"><link rel="stylesheet" href="/styles.css"><script src="/search.js" defer></script></head><body><a class="skip" href="#main">Skip to content</a><header><a class="brand" href="/"><span>▣</span> Briefcase <small>Docs</small></a><label class="search-label" for="search">Search docs<input id="search" type="search" placeholder="Search the documentation" autocomplete="off" aria-controls="search-results"></label><a class="app-link" href="https://briefcase.teamofsilicons.com">Open Briefcase ↗</a></header><div id="search-results" hidden role="region" aria-label="Search results"></div><div class="layout"><aside><span class="version">FIRST RELEASE · 1.0.0</span><nav aria-label="Documentation">${nav}</nav><a class="source" href="https://github.com/teamofsilicons/silicon-briefcase">Source on GitHub ↗</a></aside><main id="main"><div class="eyebrow">SILICON BRIEFCASE / DOCUMENTATION</div><article>${body}</article><footer>Silicon Briefcase · Contract 1.0.0 · <a href="/version-policy/">Version policy</a></footer></main><nav class="toc" aria-label="On this page"><strong>On this page</strong>${headings.map((h) => `<a href="#${h.id}">${escape(h.text)}</a>`).join("")}</nav></div></body></html>`;
...
await cp(path.join(root, "web/public/favicon.svg"), path.join(output, "favicon.svg"));
...
await writeFile(
  ...
  '<!doctype html><html lang="en"><meta charset="utf-8"><title>Page not found · Briefcase Docs</title><link rel="stylesheet" href="/styles.css"><main><h1>Page not found</h1><a href="/">Return to Briefcase documentation</a></main></html>',
);
