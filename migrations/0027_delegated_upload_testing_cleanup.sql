-- Extend logical clean/purge to privately staged delegated content. Provider
-- descriptors survive clean; only the worker may confirm their destruction.
CREATE FUNCTION briefcase.prepare_current_testing_environment_delegated_cleanup()
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    IF selected_environment IS NULL OR selected_org IS NULL
       OR selected_org NOT LIKE selected_environment::text || ':%' THEN
        RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'a selected testing environment is required';
    END IF;
    -- Exclusive environment fence is held by the caller. A writer must CAS its
    -- row/generation before every provider mutation, so revocation stops it.
    UPDATE briefcase.delegated_uploads
       SET status = 'cleanup_pending',
           cleanup_after = GREATEST(COALESCE(cleanup_after, clock_timestamp()),
           ...
           cleanup_last_error = 'testing_environment_clean'
     WHERE org_id = selected_org AND testing_environment_id = selected_environment
       AND status <> 'committed' AND provider_write_started;
    GET DIAGNOSTICS affected_rows = ROW_COUNT;
    -- Published objects are handed to entry-version cleanup by erase below.
    -- Rows that never reached a provider mutation own no external bytes.
    DELETE FROM briefcase.delegated_uploads
     WHERE org_id = selected_org AND testing_environment_id = selected_environment
       AND (status = 'committed' OR NOT provider_write_started);
    RETURN affected_rows;
END;
$$;
REVOKE ALL ON FUNCTION briefcase.prepare_current_testing_environment_delegated_cleanup() FROM PUBLIC;
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'briefcase_api') THEN
        GRANT EXECUTE ON FUNCTION briefcase.prepare_current_testing_environment_delegated_cleanup() TO briefcase_api;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'briefcase_worker') THEN
        GRANT EXECUTE ON FUNCTION briefcase.prepare_current_testing_environment_delegated_cleanup() TO briefcase_worker;
    END IF;
END;
$$;
...
BEGIN
    PERFORM briefcase.prepare_current_testing_environment_delegated_cleanup();
    IF EXISTS (SELECT 1 FROM briefcase.delegated_uploads WHERE org_id = selected_org AND provider_write_started) THEN
        RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'delegated upload provider cleanup is still pending';
    END IF;
    IF EXISTS (
