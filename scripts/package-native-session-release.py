...
def main() -> None:
    ...
    archive = output / f"briefcase-{version}.tar.gz"
    checksums = []
    expected_files = {"honeycomb.yaml"}
    with tempfile.TemporaryDirectory(prefix="briefcase-release-") as temporary:
        stage = Path(temporary)
        shutil.copyfile(ROOT / "honeycomb.yaml", stage / "honeycomb.yaml")
        for target, triple in TARGETS.items():
            name = "briefcase.exe" if target.startswith("windows-") else "briefcase"
            source = (
                args.binaries_dir / target / name
                if args.binaries_dir
                else ROOT / "clients/rust/target" / triple / "release" / name
            )
    ...
    (output / f"briefcase-{version}-binaries.sha256").write_text("\n".join(checksums) + "\n")
